...
{
    public partial class Capex : System.Web.UI.Page
    {
        ...
        private string budgetType = ConfigurationManager.AppSettings["Movable"].ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Session["user"] != null)
                {
                    ...
                    //if (User.IsInRole(AdminRole) || User.IsInRole(PBMgrRole) || User.IsInRole(PBOffRole) && budYr.IsActive.Value==true)
                    //{
                    //        if (getTrackApproval != null && getTrackApproval.Count() > 0)//if selected department has been pushed for approval
                    //        {
                ...
                if (!IsPostBack)
                {
                    ...
                    Utility.BindAssetType(ddlCat);
                    Utility.BindAssetType(ddlExpTyp);
        ...
        private void BindGrid()
        {
            ...
            if (User.IsInRole(AdminRole) || User.IsInRole(PBMgrRole) || User.IsInRole(PBOffRole))
            {
                if (ddlDeptFilter.SelectedValue != "")
                {
                    ...
                    gvDept.DataSource = CommonBLL.GetAssetBudgetList(budYr.ID,dep);
                    gvDept.DataBind();
                }
                else
                {
                    gvDept.DataSource = CommonBLL.GetAssetBudgetList(budYr.ID);
                    gvDept.DataBind();
                }
            }
            else if (User.IsInRole(EDRole) || User.IsInRole(MDRole))
            {
                if (Request.QueryString["deptId"] != null)
                {
                    ...
                    gvDept.DataSource = CommonBLL.GetAssetBudgetList(budYr.ID, deptId);
                    gvDept.DataBind();
                }
            }
            else
            {
                gvDept.DataSource = CommonBLL.GetAssetBudgetList(budYr.ID, usr.DepartmentID.Value);
                gvDept.DataBind();
            }
        }
        ...
        {
            usr = (AppUser)Session["user"]; budYr = (BudgetYear)Session["budgetYr"];
            if (User.IsInRole(AdminRole) || User.IsInRole(PBMgrRole) || User.IsInRole(deptApprRole) || User.IsInRole(MDRole))
            {
                if (Request.QueryString["deptId"] != null && Request.QueryString["batchId"] != null)
                {
                    int deptId = int.Parse(Request.QueryString["deptId"].ToString());
                    string batchId = Request.QueryString["batchId"].ToString();
                    gvATC.DataSource = CommonBLL.GetAssetBudgetATCList(batchId);
                    gvATC.DataBind();
                }
            }
        }
        ...
        {
            AssetBudget astBud = null; Asset asset = null;
            try
            {
                usr = (AppUser)Session["user"]; budYr = (BudgetYear)Session["budgetYr"];
                if (hid.Value == "Update")
                {
                   ...
                   // int month = int.Parse(ddlMonth.SelectedValue); 
                    int extTot = 0;
                    asset = LookUpBLL.GetAsset(int.Parse(ddlAsset.SelectedValue));
                    astBud = CommonBLL.GetAssetBudget(Convert.ToInt32(txtID.Text));
                    if (astBud != null)
                    {
                        ...
                        astBud.Quantity = tot;
                        astBud.AssetID = int.Parse(ddlAsset.SelectedValue);
                       ...
                       // astBud.MonthID = month;
                        ...
                        if (asset.UnitPrice.HasValue)
                        astBud.TotalCost = tot * asset.UnitPrice;
                        ...
                        //    }
                        //}
                        rst = CommonBLL.UpdateCapex(astBud);
                        if (rst != false)
                        {
                            BindGrid();
                ...
                else
                {
                  ...
                  //  astBud.MonthID = month;
                    ...
                    //}
                    if(asset.UnitPrice.HasValue)
                    astBud.TotalCost = tot * asset.UnitPrice.Value;
                    astBud.Status = (int)Utility.BudgetItemStatus.Pending_HOD_Approval;

                    result = CommonBLL.AddCapex(astBud);
                    if (result)
                    {
                        BindGrid();
                        txtTot.Text = "";
        ...
        protected void gvDept_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "del")
                {
                    ...
                    AssetBudget estf = CommonBLL.GetAssetBudget(index);
                    CommonBLL.DeleteCapex(estf);
                    BindGrid();
                    success.Visible = true;
                ...
                if (e.CommandName == "edt")
                {
                    ...
                    ddlCat.SelectedValue = estf.Asset.AssetTypeID.Value.ToString();
                    BindAssetList(estf.Asset.AssetTypeID.Value);
                    ddlAsset.SelectedValue = estf.AssetID.Value.ToString();
                    txtTot.Text = estf.Quantity.ToString();
        ...
        protected void gvDept_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lbsTot = e.Row.FindControl("lbSubTotal") as Label;
            ...
            try
            {
                ...
                foreach (GridViewRow row in gvDept.Rows)
                {
                    if (((CheckBox)row.FindControl("chkRow")).Checked)
                    {
                        ...
                        AssetBudget expPro = CommonBLL.GetAssetBudget(recID);
                ...
                if (isset)
                {
                    ...
                    if (File.Exists(FilePath + "ReturnBudget.htm"))
                    {
                        ...
                        body = body.Replace("@BudgetElement", "Movable Asset");
            ...
            try
            {
                ...
                if (isset)
                {
                    ...
                    if (File.Exists(FilePath + "RejectBudget.htm"))
                    {
                        ...
                        body = body.Replace("@add_appLogo", appLogo);
                        body = body.Replace("@siteUrl", siteUrl);
                        body = body.Replace("@BudgetElement", "Movable Budget");
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                usr = (AppUser)Session["user"]; budYr = (BudgetYear)Session["budgetYr"];
                var q = CommonBLL.GetAssetBudgetList(budYr.ID);
                if (ddlExpTyp.SelectedValue != "")
                {
                    int AstypID = int.Parse(ddlExpTyp.SelectedValue);
                    q = q.Where(p => p.Asset.AssetTypeID.Value == AstypID);
                }
                if (User.IsInRole(AdminRole) || User.IsInRole(PBMgrRole) || User.IsInRole(PBOffRole))
                {
                    if (ddlDeptFilter.SelectedValue != "")
        ...
        protected void ddlCat_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                 BindAssetList(int.Parse(ddlCat.SelectedValue));
            }
        }

        private void BindAssetList(int AssetTypeID)
        {
            ddlAsset.Items.Clear();
            ddlAsset.DataTextField = "AssetName";
            ddlAsset.DataValueField = "ID";
            ddlAsset.DataSource = LookUpBLL.GetAssetLookup(AssetTypeID);
            ddlAsset.DataBind();
        }
        ...
        protected void btnATC_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                batch = "MO-" + batch; decimal atcAmount = 0;

                foreach (GridViewRow row in gvDept.Rows)
                {
                    if (((CheckBox)row.FindControl("chkRow")).Checked)
                    {
                        Label lbAmt = row.FindControl("lbSubTotal") as Label;
                ...
                foreach (GridViewRow row in gvDept.Rows)
                {
                    if (((CheckBox)row.FindControl("chkRow")).Checked)
                    {
                        ...
                        if (expPro != null && expPro.Status == (int)Utility.BudgetItemStatus.MD_Approved)
                        {
                            ...
                            CommonBLL.UpdateCapex(expPro);
                            CommonBLL.AddAssetBudgetATC(new AssetBudgetATC() { ATCBatchId = batch, BudgetItemId = recID, BudgetItem = expPro.Asset.AssetName, ATCAmount = amt, DepartmentId = expPro.DepartmentID.Value, DateAdded = DateTime.Now, Status = (int)Utility.ATCStatus.Pending_PBManager_Approval });
                            // budgetInputer = UserBLL.GetUserByUserName(expPro.AddeBy);
                            isset = true;
                        }
                    }
                        ...
                        if (isset)
                        {
                            ...
                            if (File.Exists(FilePath + "ATCApproval.htm"))
                            {
                                ...
                                body = body.Replace("@add_appLogo", appLogo);
                                body = body.Replace("@siteUrl", siteUrl);
                                body = body.Replace("@BudgetElement", "Movable Asset Budget");
        ...
        protected void btnATCForward_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridViewRow row in gvATC.Rows)
                {
                    ...
                    AssetBudget expPro = CommonBLL.GetAssetBudget(recID);
                    AssetBudgetATC atcItm = CommonBLL.GetAssetBudgetATC(rwID);
                    if (expPro != null && expPro.ATCStatus == (int)Utility.ATCStatus.Pending_PBManager_Approval)
                    {
                        ...
                        CommonBLL.UpdateCapex(expPro);
                        CommonBLL.UpdateAssetBudgetATC(atcItm);
                        // budgetInputer = UserBLL.GetUserByUserName(expPro.AddeBy);
                        isset = true;
                    }
                    //}

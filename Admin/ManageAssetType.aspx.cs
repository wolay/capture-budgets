...
namespace BudgetCapture.Admin
{
    public partial class ManageAssetType : System.Web.UI.Page
    {
        ...
        {
            gvDir.DataSource = LookUpBLL.GetAssetTypeList();
            gvDir.DataBind();
        }
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                if (hid.Value == "Update")
                {
                    AssetType exp = null; bool rst = false;
                    exp = LookUpBLL.GetAssetType(Convert.ToInt32(txtID.Text));
                    if (exp != null)
                    {
                        ...
                        if (chkActive.Checked)
                            ...
                            exp.isActive = false;
                        rst = LookUpBLL.UpdateAssetType(exp);
                        if (rst != false)
                        {
                            BindGrid();
                ...
                else
                {
                    ...
                    if (chkActive.Checked)
                        ...
                        exp.isActive = false;
                    result = LookUpBLL.AddAssetType(exp);
                    if (result)
                    {
                        BindGrid();
                        txtDept.Text = "";
        ...
        protected void gvDir_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                hid.Value = "Update";
                dvID.Visible = true;
                dvMsg.InnerText = "Update AssetType :";

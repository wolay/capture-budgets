...
{
    public partial class IndirectBudgetPg : System.Web.UI.Page
    {
        ...
        private string budgetType = ConfigurationManager.AppSettings["Indirect"].ToString();
           ...
           // dvAdd.Visible = false; 
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    Utility.BindIndirectType(ddlCat);
                    Utility.BindIndirectType(ddlExpTyp);
        ...
        private void BindGrid()
        {
            ...
            if (User.IsInRole(AdminRole) || User.IsInRole(PBMgrRole) || User.IsInRole(PBOffRole))
            {
                if (ddlDeptFilter.SelectedValue != "")
                {
                    ...
                    gvDept.DataSource = CommonBLL.GetIndirectBudgetList(budYr.ID,dep);
                    gvDept.DataBind();
                }
                else
                {
                    gvDept.DataSource = CommonBLL.GetIndirectBudgetList(budYr.ID);
                    gvDept.DataBind();
                }
            }
            else if (User.IsInRole(EDRole) || User.IsInRole(MDRole))
            {
                if (Request.QueryString["deptId"] != null)
                {
                    ...
                    gvDept.DataSource = CommonBLL.GetIndirectBudgetList(budYr.ID,deptId);
                    gvDept.DataBind();
                }
            }
            else
            {
                gvDept.DataSource = CommonBLL.GetIndirectBudgetList(budYr.ID, usr.DepartmentID.Value);
                gvDept.DataBind();
            }
        }
        private void BindGridATC()
        {
            ...
            if (User.IsInRole(AdminRole) || User.IsInRole(PBMgrRole) || User.IsInRole(deptApprRole) || User.IsInRole(EDRole) || User.IsInRole(MDRole))
            {
                if (Request.QueryString["deptId"] != null && Request.QueryString["batchId"] != null)
                {
                    ...
                    gvATC.DataSource = CommonBLL.GetIndirectBudgetATCList(batchId);
                    gvATC.DataBind();
                }
            }
        }
        ...
        {
            IndirectBudget astBud = null; IndirectBudgetItem item = null;
            try
            {
                usr = (AppUser)Session["user"]; budYr = (BudgetYear)Session["budgetYr"];
                if (hid.Value == "Update")
                {
                    ...
                    int extTot = 0;
                    item = LookUpBLL.GetIndirectItem(int.Parse(ddlAsset.SelectedValue));
                    astBud = CommonBLL.GetIndirectBudget(Convert.ToInt32(txtID.Text));
                    if (astBud != null)
                    {
                        if (!decimal.TryParse(txtAmt.Text, out amt))
                        {
                         ...
                         ;
                        astBud.IndirectItemID = int.Parse(ddlAsset.SelectedValue);
                        ...
                        rst = CommonBLL.UpdateIndirectExpense(astBud);
                        if (rst != false)
                        {
                            BindGrid();
                ...
                else
                {
                    ...
                    result = CommonBLL.AddIndirectExpense(astBud);
                    if (result)
                    {
                        BindGrid();
                        txtAmt.Text = "";
        ...
        protected void gvDept_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "del")
                {
                    ...
                    IndirectBudget estf = CommonBLL.GetIndirectBudget(index);
                    CommonBLL.DeleteIndirectExpense(estf);
                    BindGrid();
                    success.Visible = true;
                ...
                if (e.CommandName == "edt")
                {
                    ...
                    ddlCat.SelectedValue = estf.IndirectBudgetItem.IndirectTypeID.Value.ToString();
                    BindIndirectItemList(estf.IndirectBudgetItem.IndirectTypeID.Value);
                    ddlAsset.SelectedValue = estf.IndirectItemID.Value.ToString();
            ...
            catch (Exception ex)
            {
                error.Visible = true;
                error.InnerHtml = "<button type='button' class='close' data-dismiss='alert'>&times;</button> An error occurred. kindly try again!!!";
                Utility.WriteError("Error: " + ex.StackTrace);
            }
        }
            ...
            try
            {
                ...
                foreach (GridViewRow row in gvDept.Rows)
                {
                    if (((CheckBox)row.FindControl("chkRow")).Checked)
                    {
                        ...
                        IndirectBudget expPro = CommonBLL.GetIndirectBudget(recID);
                        if (expPro != null && expPro.Status == (int)Utility.BudgetItemStatus.HOD_Approved)
                        {
                            ...
                            CommonBLL.UpdateIndirectExpense(expPro);
                ...
                if (isset)
                {
                    ...
                    if (File.Exists(FilePath + "ReturnBudget.htm"))
                    {
                        ...
                        body = body.Replace("@BudgetElement", "Indirect Budget");
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                usr = (AppUser)Session["user"]; budYr = (BudgetYear)Session["budgetYr"];
                var q = CommonBLL.GetIndirectBudgetList(budYr.ID);
                if (ddlExpTyp.SelectedValue != "")
                {
                    int AstypID = int.Parse(ddlExpTyp.SelectedValue);
                    q = q.Where(p => p.IndirectBudgetItem.IndirectTypeID.Value == AstypID);
                }
                if (User.IsInRole(AdminRole) || User.IsInRole(PBMgrRole) || User.IsInRole(PBOffRole))
                {
                    if (ddlDeptFilter.SelectedValue != "")
        ...
        protected void ddlCat_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                BindIndirectItemList(int.Parse(ddlCat.SelectedValue));
            }
        }

        private void BindIndirectItemList(int IndirectTypeID)
        {
            ddlAsset.Items.Clear();
            ddlAsset.DataTextField = "Name";
            ddlAsset.DataValueField = "ID";
            ddlAsset.DataSource = LookUpBLL.GetIndirectItemListLookUp(IndirectTypeID);
            ddlAsset.DataBind();
        }
        ...
        protected void btnATC_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                batch = "ID-" + batch; decimal atcAmount = 0;
                foreach (GridViewRow row in gvDept.Rows)
                {
                     if (((CheckBox)row.FindControl("chkRow")).Checked)
                    {
                ...
                foreach (GridViewRow row in gvDept.Rows)
                {
                    if (((CheckBox)row.FindControl("chkRow")).Checked)
                    {
                        ...
                        if (expPro != null && expPro.Status == (int)Utility.BudgetItemStatus.MD_Approved)
                        {
                            ...
                            CommonBLL.UpdateIndirectExpense(expPro);
                            CommonBLL.AddIndirectExpenseATC(new IndirectBudgetATC() {ATCBatchId=batch,BudgetItemId=recID, BudgetItem=expPro.IndirectBudgetItem.Name, ATCAmount=amt,DepartmentId=expPro.DepartmentID.Value,DateAdded=DateTime.Now,Status=(int)Utility.ATCStatus.Pending_PBManager_Approval });
                            // budgetInputer = UserBLL.GetUserByUserName(expPro.AddeBy);
                            isset = true;
                        }
                    }
        ...
        protected void btnATCForward_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridViewRow row in gvATC.Rows)
                {
                    ...
                    IndirectBudget expPro = CommonBLL.GetIndirectBudget(recID);
                    IndirectBudgetATC atcitm = CommonBLL.GetIndirectBudgetATC(rwID);
                    if (expPro != null && expPro.ATCStatus == (int)Utility.ATCStatus.Pending_PBManager_Approval)
                    {
                        ...
                        CommonBLL.UpdateIndirectExpense(expPro);
                        CommonBLL.UpdateIndirectExpenseATC(atcitm);
                        // budgetInputer = UserBLL.GetUserByUserName(expPro.AddeBy);
                        isset = true;
                    }
                    //}
